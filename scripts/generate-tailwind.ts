...
const main = async () => {
  ...
  const tailwindFontsGroupsRegex = /^\.font-([^-]+(?:-[^-]+)*)-\d+$/;
  const tailwindFontsGroups = [
    ...new Set(
      tailwindFontsClasses
        .map((className) => className.match(tailwindFontsGroupsRegex)?.[1])
...
export const tailwindFontsGroups = [${tailwindFontsGroups
